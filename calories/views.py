...
def HomePageView(request):
	...
	calories = Food_Profile.objects.filter(person_of=request.user).last()
	calorie_goal = calories.calorie_goal
	
	#creating one profile each day
	if date.today() > calories.date:
		profile=Food_Profile.objects.create(person_of=request.user)
		profile.save()
	calories = Food_Profile.objects.filter(person_of=request.user).last()
...
def select_food(request):
	person = Food_Profile.objects.filter(person_of=request.user).last()
...
def ProfilePage(request):
	...
	records= Food_Profile.objects.filter(date__gte=some_day_last_week,date__lt=timezone.now().date(),person_of=request.user)
