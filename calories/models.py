...
class Food_Profile(models.Model):
	person_of = models.ForeignKey(User,null=True,on_delete=models.CASCADE)
	...
	def save(self, *args, **kwargs):# new
		if self.food_selected != None:
			...
			calories = Food_Profile.objects.filter(person_of=self.person_of).last()
			...
			super(Food_Profile, self).save(*args,**kwargs)
...
class PostFood(models.Model):
    profile = models.ForeignKey(Food_Profile, on_delete=models.CASCADE)
